...
if __name__ == '__main__':
    data_set = pull_dataset('sentences.txt')
    print(data_set[:5])  # Check loaded data set
    time.sleep(10)
    create_and_train_model(data_set, 'word2vec_model')

    model = load_model('word2vec_model')
    print(len(model.vocab))
    print(model.most_similar(positive['python'], topn=10))
    print(model.most_similar(positive['python'], negative=['java'], topn=10))
